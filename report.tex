...
\texttt{Sketch up 3D Warehouse}\cite{sketchup}, remove the trunk and branches as
much as possible and then wrap them using the 
\texttt{CGAL Alpha Wrapper}\cite{cgal_alpha_wrapper} algorithm (explained in the 
next section \autoref{section:alpha_wrapping} ). \\
Here is an example of a Ginkgo tree mesh:
\begin{figure}[H]
    \centering
        \centering
...
Where 50 is the \texttt{alpha} value and 600 is the \texttt{offset} value. (cf \autoref{sec:alpha_wrapping})
...
performance. Here is the number of faces for each tree type and LOD:
